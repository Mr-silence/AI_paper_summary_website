# AI论文简报 - 详细需求与系统设计文档 (PRD) - v2.18 (全量无损收口版)
...
**唯一选择权**: 系统排序引擎 (Scorer) 拥有绝对的候选论文“入选权”。AI 环节 (Editor) 仅负责对已入选论文生成定调简报，无权剔除或增加。
...
2.  **强制截断与归档 (Capacity & Archiving)**: 
    ...
    *   **Candidate 归档定义**: 满足以下任一条件的论文在 `paper_summary` 中标记为 `category = 'candidate'`：
        *   评分 $\ge 50$ 但在 Top 5/12 容量限制之外的论文。
        *   评分 $< 50$ 的全量抓取论文。
        *   进入了解读流但最终被 Reviewer 标记为 `REJECTED` 的论文。
3.  **质量基准线 (Quality Baseline)**: 
    ...
    *   **后置审计与补位状态迁移 (Backfill & State Transition)**: 
        ...
        *   **正向迁移**: 补位流程通过 **UPDATE (原子更新)** 将记录从 `candidate` 变迁至 `focus/watching`，填充 narrative 字段。
        *   **逆向迁移**: 若 Reviewer 拒绝某篇论文，系统必须通过 **UPDATE (原子更新)** 将其 `category` 设为 `candidate`，且必须将所有 narrative 解读字段**物理重置为 NULL**。
...
| 信号 | 加分 | 触发逻辑 |
| :--- | :--- | :--- |
| **顶尖机构** | +20 | 匹配内置 40+ 机构库 (Google, OpenAI, Meta, 清华, 北大等)。 |
...
| **代码可用** | +20 | 摘要包含 github.com 链接或 Official Code 声明。 |
...
*   **输出模板 (强制执行)**: 
    ...
    ```
*   **提取正则与重试规则**: 
    *   **Block 切分**: `blocks = re.split(r"## 论文: \[(.*?)\]", output)`。
    *   **属性提取 (针对每篇 block)**: `re.search(r"- \*\*写作角度\*\*: (.*?)\n", block)`, `re.search(r"- \*\*核心痛点\*\*: (.*?)\n", block)`, `re.search(r"- \*\*具体解法\*\*: (.*?)\n", block)`。
...
*   **输出模板 (强制执行)**:
    ...
    ```
*   **机械级解析规则**:
    1.  **Block 切分**: `blocks = re.split(r"## \[(.*?)\]", writer_output)`。
    2.  **属性提取 (针对 block)**:
...
**物理 NULL 约束**: 当 `category = 'candidate'` 时，表中的解读字段 (8-13) **必须存储为物理 NULL 值**。
| 字段 | 类型 | 约束 | 描述 |
| :--- | :--- | :--- | :--- |
...
| 7. `direction` | ENUM('Agent', 'Reasoning', 'Training_Opt', 'RAG', 'Multimodal', 'Code_Intelligence', 'Vision_Image', 'Video', 'Safety_Alignment', 'Robotics', 'Audio', 'Interpretability', 'Benchmarking', 'Data_Engineering', 'Industry_Trends') | INDEX, NOT NULL | 分类快照 |
| 8. `one_line_summary` | VARCHAR(255) | NULL | 中文一句话总结 |
| 9. `one_line_summary_en` | VARCHAR(255) | NULL | 英文一句话总结 |
| 10. `core_highlights` | JSON | NULL | 中文核心亮点 (List) |
| 11. `core_highlights_en` | JSON | NULL | 英文核心亮点 (List) |
| 12. `application_scenarios` | TEXT | NULL | 中文应用场景 |
| 13. `application_scenarios_en`| TEXT | NULL | 英文应用场景 |
...
1.  **GET /api/v1/papers** (列表分页)
    ...
    *   **Response Payload**: `{ "total": INT, "items": [ { "id", "arxiv_id", "title_zh", "title_original", "score", "category", "direction", "issue_date", "one_line_summary", "one_line_summary_en" } ] }`。
2.  **GET /api/v1/papers/{id}** (单篇详情)
    *   **Response Payload**: 包含列表字段 + `abstract`, `authors`, `score_reasons`, `core_highlights`, `core_highlights_en`, `application_scenarios`, `application_scenarios_en`。
    *   **NULL 契约**: 若目标论文 `category == 'candidate'`，响应中所有的解读字段必须显式返回 `null`。
